...
def save_history(history: tf.keras.callbacks.History, output: Path) -> None:
    for metric, title in (("accuracy", "Model accuracy"), ("loss", "Model loss")):
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(output / f"{metric}.png", dpi=160)
        plt.close()


def evaluate(
    model: tf.keras.Model, dataset: tf.data.Dataset, labels: list[str], output: Path
) -> None:
    ...
    fig, ax = plt.subplots(
        figsize=(max(12, len(labels) * 0.35), max(10, len(labels) * 0.35))
    )
    ConfusionMatrixDisplay.from_predictions(
        y_true, y_pred, display_labels=labels, xticks_rotation=90, ax=ax, colorbar=False
    )
    fig.tight_layout()
...
def main() -> None:
    ...
    model_dir, eval_dir = root / "models", root / "evaluation"
    ...
    save_history(history, eval_dir)
    model = tf.keras.models.load_model(checkpoint)
    evaluate(model, val_ds, labels, eval_dir)
