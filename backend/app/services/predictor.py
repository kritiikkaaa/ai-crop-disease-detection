...
class Predictor:
    ...
    def _load(self, model_path: str, labels_path: str) -> None:
        ...
        signature = (str(model_file), model_file.stat().st_mtime, str(labels_file), labels_file.stat().st_mtime)
        with self._lock:
    ...
    def predict(self, raw: bytes, model_path: str, labels_path: str) -> dict:
        ...
        return {"label": label, "disease": label.replace("___", " — ").replace("_", " "), "confidence": round(float(probabilities[ranked[0]]) * 100, 2), "top_predictions": [{"disease": self._labels[int(i)].replace("___", " — ").replace("_", " "), "confidence": round(float(probabilities[i]) * 100, 2)} for i in ranked]}
